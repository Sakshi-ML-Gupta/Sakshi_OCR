...
def _chunk_lines_by_char_budget(numbered_lines: list, questions: list,
    ...
    label (Ans-, उत्तर-, etc.) OR a line whose opening words overlap
    substantially with an official question's opening words (the
    "student restates the question, no label" pattern). This means a
    chunk boundary can never fall in the middle of a single answer's
    content UNLESS that one answer alone is so long it exceeds even
    the hard cap, in which case a forced break is the only sane
    fallback.
    """
