...
# =========================================================
# FIX 1: ANSWER_MAP_MAX_CHARS_PER_CHUNK raised from 11000 -> 25000
#
# A single 10-page handwritten Hindi answer in Devanagari OCR can
# easily be 15k-22k chars. At 11000, a long answer gets split across
# two chunks: each chunk only sees HALF the answer, so the LLM maps
# a short line range for each half — and only the best (longest) range
# wins dedup, producing a truncated answer that's roughly half the
# real content. 25000 keeps the vast majority of individual long
# answers within a single chunk so the LLM sees the complete picture.
# The existing 413/429 retry-with-backoff in _call_groq_with_retries
# handles the rare oversized chunk gracefully — slower, but never
# silently truncates.
# =========================================================
ANSWER_MAP_MAX_CHARS_PER_CHUNK = 25000

# =========================================================
# FIX 2: ANSWER_MAP_ABSOLUTE_MAX_CHARS raised from 60000 -> 120000
#
# Paired with the target above. 120000 chars is roughly 20+ pages of
# Devanagari OCR — a true last-resort ceiling that should never be
# reached for any real exam booklet.
# =========================================================
ANSWER_MAP_ABSOLUTE_MAX_CHARS = 120000
...
def strip_full_question_echo(answer_text: str, question_text: str) -> str:
    """
    Strips a verbatim question restatement from the start of an answer,
    but ONLY when the echo is very clear:

    FIX 1: min_n floor raised from 3 -> 6 words.
      Stripping fewer than 6 words is almost always a false positive
      on short shared topic phrases like "theme of X" or "role of Y".

    FIX 2: similarity threshold raised from 0.75 -> 0.88.
      0.75 was eating the actual first sentence of answers that merely
      shared vocabulary with the question (e.g. an answer about
      "concealment" opening with "concealment is a theme...").
      0.88 requires near-verbatim reproduction to qualify.

    FIX 3: length guard — don't strip if the matched window would
      consume more than 60% of the answer. If "stripping the echo"
      would eat most of the answer, it's real content, not a restatement.
    """
    ...
    # FIX 1: floor raised 3 -> 6
    min_n = max(6, int(q_word_count * 0.7))
    ...
    for n in range(min_n, max_n + 1):
        ...
        # FIX 2: threshold raised 0.75 -> 0.88
        if ratio >= 0.88 and ratio > best_ratio:
            best_ratio = ratio
            best_strip_count = n

    # FIX 3: don't strip if the echo would consume >60% of the answer
    if best_strip_count > 0 and best_strip_count < len(answer_words) * 0.6:
...
# =========================================================
# FIND QUESTION BOUNDARIES -- similarity based (legacy, unchanged)
# =========================================================
