...
# =========================================================
LLM_PROVIDER = os.getenv("LLM_PROVIDER", "anthropic").lower()  # "anthropic" (default) or "groq"
...
class _RotatingLLMClient:
    ...
    def __init__(self, api_keys: list, budget: "_TokenBudgetTracker" = None, log=print):
        if not api_keys:
            raise Exception(f"No {LLM_PROVIDER.upper()} API key(s) configured")
        self._keys = api_keys
        self._index = 0
        self._budget = budget
        self._log = log
...
def _is_near_duplicate_question(q1: str, q2: str) -> bool:
    """
    Only an EXACT match (after normalizing case/whitespace/leading numbering) counts
    as a duplicate. Fuzzy similarity was tried before and is deliberately NOT used --
    sibling sub-parts of a multi-part question legitimately carry the SAME parent
    instruction text forward into each sub-part (e.g. "1.(i) Identify and explain:
    theme of light." vs "1.(ii) Identify and explain: theme of darkness.") and easily
    cross a fuzzy-similarity threshold despite being genuinely separate questions --
    that was a confirmed cause of a real question silently vanishing from the list.
    """
...
def extract_canonical_questions(qp_pages: list, status_callback=None) -> list:
    ...
    if not api_keys:
        raise Exception(f"No {LLM_PROVIDER.upper()} API key found in secrets or environment")
