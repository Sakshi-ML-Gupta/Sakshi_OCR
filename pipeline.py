...
class GroqQuotaExhausted(Exception):
    ...
    configured API keys -- never caught by generic except blocks,
    always propagates immediately so the pipeline stops the instant
    every available key is exhausted, instead of grinding through
    guaranteed-to-fail retries.
    """
    pass
...
class _GroqClientPool:
    """
    Manages GROQ_API_KEY plus any GROQ_API_KEY2, GROQ_API_KEY3, ...
    found in secrets/environment, and transparently switches to the
    next available key the INSTANT the current one's daily quota (TPD)
    is exhausted -- mid-call, mid-document. All pipeline state (search
    pointer, found_starts, ranges) lives outside the client, so
    swapping clients and retrying the SAME request resumes exactly
    where processing left off -- zero restart, zero lost work.

    TIME OPTIMIZATION: each key gets its OWN independent per-minute
    (TPM) token-budget tracker (self._budgets), instead of every key
    sharing one global pacing clock. Previously, configuring 2-3 keys
    only helped once a key's DAILY quota ran out -- per-minute pacing
    waits ("Proactively pacing requests... Waiting 60.5s") still
    applied as if there were only one key, since a single shared
    _TokenBudgetTracker object was reused across every key. Now, when
    key #1 is near its per-minute cap, key #2 (if configured) has its
    OWN, still-empty budget -- so multiple keys genuinely multiply
    effective throughput, not just provide a same-speed fallback.
    """
    def __init__(self, log=print):
        ...
        if not keys:
            raise Exception(
                "No GROQ_API_KEY (or GROQ_API_KEY2, GROQ_API_KEY3, ...) found "
                "in secrets or environment"
            )
        self._keys = keys
        self._clients = [Groq(api_key=k) for k in keys]
        self._budgets = [_TokenBudgetTracker() for _ in keys]
        self._current = 0
        if len(keys) > 1:
            self.log(
                f"Groq fallback enabled: {len(keys)} API key(s) configured, "
                f"each with its own independent per-minute token budget."
            )
    ...
    @property
    def budget(self) -> "_TokenBudgetTracker":
        """The CURRENTLY ACTIVE key's own per-minute budget tracker."""
        return self._budgets[self._current]
...
def _call_groq_with_retries(client, system_prompt: str, user_prompt: str,
    ...
    """
    `client` here is a _GroqClientPool -- this is the ONLY function that
    needs to know about the pool; every other caller just passes it
    through opaquely.

    TIME OPTIMIZATION: the `budget` parameter (kept for backward
    compatibility with every caller in this module) is intentionally
    NOT used for pacing here anymore. Instead, every iteration reads
    `client.budget` fresh -- the per-minute tracker belonging to
    whichever key is CURRENTLY active. This means that when a key is
    switched via client.advance() (on daily-quota exhaustion), the very
    next loop iteration automatically starts pacing against the NEW
    key's own, still-empty per-minute budget -- so configuring 2-3 keys
    genuinely multiplies effective throughput instead of just providing
    a same-speed fallback.
    """
    import groq
    ...
    while attempt <= max_retries + 1:
        active_budget = client.budget

        if skip_next_proactive_check:
            skip_next_proactive_check = False
        else:
            active_budget.wait_if_needed(estimated_tokens, log=log)
        ...
        try:
            ...
            active_budget.record_usage(estimated_tokens)
            ...
            if detail and detail["limit_type"] == "TPD":
                if client.advance():
                    # Switched to a fallback key -- the NEXT loop
                    # iteration will fetch client.budget fresh (the new
                    # key's own tracker), so no manual reset is needed
                    # here. Retry the SAME request without consuming a
                    # retry slot. Free.
                    continue
            ...
            if detail:
                active_budget.record_actual_from_error(detail["used"], detail["limit"])
                log(
...
#
# IMPORTANT INVARIANT: search calls are never artificially bounded by
# "the nearest already-known future anchor" -- an earlier version of
# this file did this as a token-saving optimization, but it was a
# confirmed, reproduced bug: if a FUTURE question's anchor (from the
# regex label pre-pass or an earlier out-of-order resolution) was ever
# wrong -- pointing to an earlier line than the question's true answer
# -- every question BETWEEN the current search pointer and that wrong
# anchor got its search window silently truncated before it, so their
# real content was never found and ended up unmatched, then silently
# absorbed into whichever answer preceded them. Do not reintroduce an
# end_idx bound derived from other questions' found_starts here.
# =========================================================
...
- IGNORE OCR ARTIFACT-DESCRIPTION LINES completely when judging where an answer starts or ends -- e.g. a line that just describes a non-text element such as "there is a logo", "red pen scribble", "signature", "watermark", "stamp here". These are never part of the student's actual written answer; skip over them as if they were not there, and never report one of these lines as start_line.
- NEVER let two DIFFERENT questions' answers get merged into one match. If this window contains the tail end of a DIFFERENT (earlier or later) question's answer immediately next to content that addresses the target question, report start_line as the EXACT line where the target question's own content begins -- not one line earlier or later, and not a line that still belongs to the other question.
- PRECISION MATTERS MOST: if you are not reasonably confident a line is the genuine start of THIS question's answer, report found=false rather than guessing. A missed match can be found later by another pass; but wrongly matching a line that actually belongs to a DIFFERENT question means that other question's content gets mixed into this one's answer -- a much worse, silent error. When in doubt, do not match.
...
def _find_answer_start_sequential(client, numbered_lines: list, question_text: str, ref_label: str,
                                    ...
                                    cache: dict = None):
    """
    `cache`, if provided, is a dict shared across every search call for
    the CURRENT document. Before spending a Groq call on a given
    (ref_label, window bounds, reminder) combination, it checks the
    cache first -- this avoids re-spending tokens/time on a window
    that's already been asked about with the exact same context, which
    happens whenever the rescue/reanalyze/remap passes re-search a
    region that the initial forward pass (or another pass) already
    covered.
    """
...
def _rescue_unmatched_questions(client, numbered_lines: list, questions: list, ranges: list,
                                  budget: "_TokenBudgetTracker", log, cache: dict = None) -> list:
    """
    ...
    NEIGHBORING matched question's range. This does a narrowly BOUNDED
    re-search only inside the range that currently spans across where
    the missing one should be, and splits that range if found. Runs
    multiple passes so a RUN of several consecutive unmatched questions
    gets recovered one at a time. Each attempt also gets ONE retry with
    an explicit reminder if the first bounded search comes up empty.

    FIX: previously, a question with NO EARLIER matched neighbor (this
    is always the case for the FIRST question in the document if its
    own initial forward search happened to fail) could NEVER be
    rescued -- the old code required a preceding matched range to
    search "inside", and simply skipped the question forever if none
    existed. This is a confirmed, reproducible cause of "N-1 out of N
    questions found" even when the missing question's content is
    plainly present in the document. Now, when there's no preceding
    matched range, the search instead runs from the very start of the
    document up to whatever the NEXT matched question's start line is
    (or to the end of the document, if nothing at all is matched yet).
    """
    ...
    while changed and passes < 4:
        ...
        for i, q in enumerate(questions):
            ...
            if candidate is not None:
                ...
                bounded_source = candidate["ref"]
            else:
                # No preceding matched range at all (most commonly: this
                # IS question #1 and its own initial search failed).
                # Search from the very start of the document up to
                # whatever the closest FOLLOWING matched question starts
                # at -- so this question can still be recovered instead
                # of being silently skipped forever.
                next_candidate = None
                for r in ranges:
                    r_idx = ref_to_idx.get(r["ref"])
                    if r_idx is not None and r_idx > i:
                        if next_candidate is None or r_idx < ref_to_idx[next_candidate["ref"]]:
                            next_candidate = r
                lower = 0
                upper = (next_candidate["start_line"] - 1) if next_candidate is not None else (total_lines - 1)
                bounded_source = next_candidate["ref"] if next_candidate is not None else "(document start)"

            if upper <= lower:
                continue

            log(f"  RESCUE: {ref} is unmatched -- searching for it in lines {lower}-{upper} (bounded by {bounded_source})...")
...
def _remap_incomplete_answers(client, numbered_lines: list, questions: list, ranges: list,
                                budget: "_TokenBudgetTracker", log, max_passes: int = 1,
...
def _build_label_anchor_index(answer_lines: list, questions: list, exclude_indices: set, log=print) -> dict:
    """
    Deterministic (zero-LLM-cost) label pre-pass. Scans the whole
    document ONCE with a regex for lines carrying an explicit answer
    label ("Ans 5-", "उत्तर 6-", "Q.7", etc.) whose number matches a
    STANDALONE question's own leading number, in strictly increasing
    document order. NOTE: these anchors are used ONLY as a starting
    point for found_starts (still subject to boundary re-checks) -- they
    are never used to BOUND another question's search window (that was
    a confirmed bug; see the long comment above SEQUENTIAL_SEARCH_SYSTEM_PROMPT).
    """
    q_number_to_indices = {}
    ...
    if anchors:
        log(
            ...
            f"question(s) via explicit labels (Ans/उत्तर/Q. etc.) as a starting point -- "
            f"these still go through the normal boundary-check pass."
        )
    return anchors
...
def map_answers_sequential(answer_lines: list, questions: list, status_callback=None,
    ...
    previous question's answer was confirmed to start, for a line
    where THIS question's answer begins. Once found, the previous
    question's END is computed as (this start - 1) -- never asked of
    the LLM. If a question's start can't be found, it is recorded as
    unmatched and the search POINTER IS NOT ADVANCED.
    """
    def log(msg):
        print(msg)
        if status_callback:
    ...
    # Shared across EVERY search call for this document -- avoids
    # re-spending tokens/time on a window that's already been asked
    # about with the exact same (ref, window, reminder) combination
    # (this happens whenever the rescue/reanalyze/remap passes below
    # revisit territory the initial forward pass already covered).
    search_cache = {}
    ...
    try:
        while i < n:
            if i in sibling_groups:
                ...
                group_start = _find_answer_start_sequential(client, numbered_lines, first_q, first_ref, pointer, budget, log, cache=search_cache)
                ...
                if group_start is None:
                    ...
                    group_start = _find_answer_start_sequential(
                        ...
                        extra_reminder=reminder, cache=search_cache
                    )
                ...
                if next_index < n:
                    ...
                    group_end_bound = _find_answer_start_sequential(
                        client, numbered_lines, next_q, next_ref, group_start + 1, budget, log, cache=search_cache
                    )
            ...
            # NOTE: deliberately NOT bounded by any "future anchor" --
            # see the long comment above SEQUENTIAL_SEARCH_SYSTEM_PROMPT
            # for why that optimization was removed (it caused answers
            # to be silently absorbed into a neighboring range).
            start_line = _find_answer_start_sequential(
                client, numbered_lines, q, ref, pointer, budget, log,
                extra_reminder=sub_part_hint, cache=search_cache
            )

            if start_line is None:
                log(f"  pass 1 found nothing for {ref} -- retrying with a stronger reminder...")
                reminder_parts = [
                ...
                start_line = _find_answer_start_sequential(
                    client, numbered_lines, q, ref, pointer, budget, log,
                    extra_reminder="\n\n".join(reminder_parts), cache=search_cache
                )
    ...
    # RESCUE PASS: recover any question whose content silently got
    # absorbed into a neighboring matched question's range.
...
def process_pdf(file_input, status_callback=None):
    ...
    try:
        ...
        raise Exception(
            f"Processing stopped: {e}\n\n"
            f"Wait for the daily quota reset, then re-run -- OCR results are "
            f"cached, so re-run resumes quickly. Consider adding a "
            f"GROQ_API_KEY2 (and further) to secrets/environment for "
            f"automatic fallback capacity."
        ) from e
