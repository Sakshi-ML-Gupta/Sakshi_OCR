...
def map_answers_with_llm(answer_lines: list, questions: list, status_callback=None) -> dict:
    ...
    # the longer range (more complete capture). Uses the same fuzzy
    # normalization as question dedup, since the LLM echoing back the
    # question text could introduce small formatting differences even
    # though it was given the exact canonical text to use.
    best_by_question = {}
    for r in all_ranges:
        key = _normalize_question_key(r["question"])
