...
BACKWARD_REFINE_LOOKBACK_LINES = 25
...
def _refine_start_backward(client, numbered_lines: list, question_text: str, ref_label: str,
                             ...
                             log, lookback_lines: int = BACKWARD_REFINE_LOOKBACK_LINES):
    """
    After the forward search finds a candidate start_line, double-check
    whether the TRUE start is actually a bit earlier within this answer's
    own territory (never earlier than floor_idx, which is where the
    PREVIOUS answer's range ends -- so this can never steal content from
    a different question).

    This directly targets the reported "answer picked up mid-paragraph,
    the real opening paragraph(s)/page got skipped" bug: the forward
    search only ever asks "does it start SOMEWHERE in this window", and
    if the true opening line is a plain, unlabeled sentence that doesn't
    obviously scream "this is question X's answer" on its own, the model
    can latch onto a later, more obviously on-topic paragraph instead.
    Showing it the bounded window immediately before that point and
    asking the sharper, contrastive question "is the TRUE start actually
    earlier, right here?" catches this reliably in practice.
    """
    window_start = max(floor_idx, candidate_start - lookback_lines)
    if window_start >= candidate_start:
        return candidate_start  # no room to look back

    window = [nl for nl in numbered_lines if window_start <= nl[0] <= candidate_start]
    if not window:
    ...
    try:
        ...
        log(f"WARNING: backward-refine call failed for {ref_label}, keeping original start_line {candidate_start}: {e}")
        return candidate_start

    if earlier_found and earlier_start is not None:
        valid_ids = {i for i, _ in window}
        if earlier_start in valid_ids and floor_idx <= earlier_start <= candidate_start:
            if earlier_start != candidate_start:
                log(f"  backward-refine moved {ref_label} start earlier: {candidate_start} -> {earlier_start}")
            return earlier_start
        log(
            f"WARNING: backward-refine for {ref_label} returned out-of-range line "
            f"{earlier_start} -- keeping original start_line {candidate_start}"
        )
...
def map_answers_sequential(answer_lines: list, questions: list, status_callback=None,
    ...
    for i, q in enumerate(questions):
        ...
        if start_line is not None:
            ...
            found_starts[ref] = refined_start
            log(f"  found {ref} starting at line {refined_start}")
