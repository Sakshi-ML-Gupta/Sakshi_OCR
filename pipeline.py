...
class SemanticMatcher:
    ...
    def clean_answer_semantically(self, answer_text: str, question_text: str, 
        ...
        # First clean unwanted text patterns
        answer_text = clean_unwanted_text(answer_text)
        ...
        for sentence in sentences:
            # Skip very short sentences that are likely noise
            ...
            # Keep sentence if not too similar to question OR it's very short (likely noise)
            if similarity < threshold or len(sentence.split()) < 3:
                cleaned_sentences.append(sentence)
    ...
    def clean_qa_pairs(self, qa_pairs: list, status_callback=None) -> list:
        ...
        for pair in qa_pairs:
            ...
            if answer:
                ...
                # Apply extra cleanup for specific patterns
                cleaned_answer = clean_unwanted_text(cleaned_answer)
...
def map_answers_sequential(answer_lines: list, questions: list, status_callback=None,
    ...
    for i, q in enumerate(questions):
        ...
        # Clean unwanted text patterns
        answer_clean = clean_unwanted_text(answer_clean)
...
def process_pdf(file_input, status_callback=None):
    ...
    for page in answer_pages:
        for line in page["raw_text"].split("\n"):
            if not is_noise(line):
                # Clean unwanted text patterns from each line
                line = clean_unwanted_text(line)
    ...
    # Apply semantic cleaning to remove question restatements from answers
...
# Global instance for semantic matcher
