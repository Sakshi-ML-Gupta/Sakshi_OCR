...
def _guarantee_full_mapping(client, numbered_lines: list, questions: list, ranges: list,
    ...
    return ranges
# =========================================================
# FINAL SAFETY NET: split on embedded labels (zero-LLM-cost)
#
# The clearest possible signal that two answers got merged into one
# range is an explicit label for a DIFFERENT question ("Ans 6-", "Q.7",
# "उत्तर 8") appearing partway THROUGH an already-finalized range,
# rather than at its start. This scans every range for exactly that
# and splits on sight -- purely deterministic regex matching, no LLM
# call at all, so it runs even if the Groq quota is fully exhausted
# and can never itself fail/cost tokens.
# =========================================================
def _split_ranges_on_embedded_labels(answer_lines: list, questions: list, ranges: list, log) -> list:
    ...
    for qi, q in enumerate(questions):
        qn = _extract_leading_number(q)
        if qn is not None:
            q_number_to_idx.setdefault(qn, []).append(qi)
    changed = True
    passes = 0
    while changed and passes < 3:
        ...
        ordered = sorted(ranges, key=lambda r: r["start_line"])
        for r in ordered:
            own_qi = _ref_to_question_index(r["ref"])
            s, e = r["start_line"], r["end_line"]
            for j in range(s + 1, e + 1):  # skip the range's own first line (its own label)
                ...
                if not num_match:
                    continue
                candidate_qis = [qi for qi in q_number_to_idx.get(num_match.group(0), []) if qi != own_qi]
                if not candidate_qis:
                    continue
                target_qi = candidate_qis[0]
                target_ref = f"REF-{chr(65 + target_qi)}"
                if any(x["ref"] == target_ref for x in ranges):
                    continue  # that question is already matched elsewhere -- don't create a duplicate
                log(
                    f"  SAFETY-SPLIT: found an explicit label for {target_ref} embedded inside "
                    f"{r['ref']}'s range at line {j} (two answers were merged) -- splitting: "
                    f"{r['ref']} now ends at line {j - 1}, {target_ref} takes lines {j}-{e}."
                )
                r["end_line"] = j - 1
                ranges.append({"ref": target_ref, "start_line": j, "end_line": e})
                changed = True
                break
            if changed:
                break
...
def map_answers_sequential(answer_lines: list, questions: list, status_callback=None,
    ...
    # FIX (root-cause): label anchors used to be trusted BLINDLY here,
    # with zero verification -- if a student's own answer happened to
    # reference an earlier question by number in prose (e.g. a line
    # starting "उत्तर 5 mein maine..." while still writing question 7's
    # answer), that incidental match would silently become a WRONG,
    # completely unverified anchor for question 5, and no later pass
    # ever re-checks an already-"found" ref. This was a confirmed,
    # content-dependent cause of answers ending up mixed/wrong on some
    # documents but not others. Anchors are now kept as CANDIDATES only
    # -- each one still goes through the same cheap, single-call
    # boundary verification used for real search results before being
    # accepted, in the main loop below. This keeps the expensive part
    # of the token savings (skipping the full multi-window search) while
    # closing the zero-verification gap.
    found_starts = {}
    pointer = 0
    i = 0
    n = len(questions)
    ...
    while i < n and not quota_exhausted:
      try:
        ...
        future_confirmed = [v for k, v in found_starts.items() if _ref_to_question_index(k) > i]
        future_candidates = [v for qi2, v in label_anchors.items() if qi2 > i]
        future_lines = future_confirmed + future_candidates
        bound_end_idx = (min(future_lines) + 1) if future_lines else None
        anchor_candidate = label_anchors.get(i)
        if anchor_candidate is not None and anchor_candidate >= pointer:
            # FIX (root-cause): a deterministic label match exists, but
            # it is NOT trusted blindly anymore -- run it through the
            # same cheap, single-call boundary check used for real
            # search results. This still skips the expensive multi-
            # window search (the main cost/time saving), but closes the
            # zero-verification gap that let an incidental in-prose
            # number reference silently become a wrong, unchecked anchor.
            log(f"Verifying label-anchor candidate for {ref} at line {anchor_candidate}...")
            start_line = _check_boundary_combined(
                client, numbered_lines, questions[i - 1] if i > 0 else None, q,
                anchor_candidate, search_from_idx, budget, log
            )
            log(f"  {ref}: label-anchor candidate at line {anchor_candidate} -> verified start at line {start_line}")
        else:
    ...
    # Runs regardless of quota status -- purely deterministic regex
    # matching, no API call, so it works even when everything above had
    # to stop early.
    ranges = _split_ranges_on_embedded_labels(answer_lines, questions, ranges, log)
